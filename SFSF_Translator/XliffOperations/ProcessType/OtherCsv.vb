...
Public Class OtherCsv
    ...
    Public Sub CreateOutFile() Implements iXliff.CreateOutFile
        Try
            If File.Exists(CPS.OutFile) Then
                ...
                UpdateMsg(Now & Chr(9) & "Multilingual Other csv. File has already been integrated. Adding language. pleast wait..." & vbCrLf, Form_MainNew.RtbColor.Black)
                Mod_QuestionLib.ToCsv(CPS.OutFile, CPS.Xliff_FileInBackFromtranslation, CPS.CurrentLang, _BW)
            Else
                'first language we integrate => we use the original file
                UpdateMsg(Now & Chr(9) & "Multilingual Other csv. Integrating first language. please wait..." & vbCrLf, Form_MainNew.RtbColor.Black)
                Mod_QuestionLib.ToCsv(_sfile, CPS.Xliff_FileInBackFromtranslation, CPS.CurrentLang, _BW)
            End If
    ...
    Public Function ExtractXliff() As Boolean Implements iXliff.ExtractXliff
        Try
            Msg = Mod_QuestionLib.ToXliff(_sfile, CloudProjectsettings.Folder_TobeTransalted, CPS.CurrentLang, CPS._PD.isMaxLengthCheckRequired)
            If Msg <> "" Then
    ...
    Public Sub ImportExistingTranslationToDB() Implements iXliff.ImportExistingTranslationToDB
        UpdateMsg(Now & Chr(9) & "DB update not implemented for OtherCsv type" & vbCrLf, Form_MainNew.RtbColor.Red)
    End Sub

    Public Sub ExtractXiffwithExistingTranslation() Implements iXliff.ExtractXiffwithExistingTranslation
        UpdateMsg(Now & Chr(9) & "ExtractXiffwithExistingTranslation not implemented for OtherCsv type" & vbCrLf, Form_MainNew.RtbColor.Red)
    End Sub
End Class
