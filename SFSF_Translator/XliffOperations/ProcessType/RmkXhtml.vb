...
Public Class RmkXhtml
    ...
    Public Sub InitializeCurrentProjectSetting(ByRef _CPS As CloudProjectsettings)
        CPS = _CPS
    End Sub
    ...
    Public Sub StartProcessing(sFile As String, ByRef BW As BackgroundWorker) Implements iXliff.StartProcessing
        ...
        UpdateMsg(Now & Chr(9) & "File Type - RMK xml - Category Job" & vbCrLf, Form_MainNew.RtbColor.Black)
        ...
        Try
            ...
            AddHandler IP.InitializeCurrentProjectSetting, AddressOf InitializeCurrentProjectSetting
    ...
    Public Sub CreateOutFile() Implements iXliff.CreateOutFile
        Try
            Mod_RMK_xhtml.xliff_to_RMK(_sFile, CPS.Xliff_FileInBackFromtranslation, CPS.CurrentLang)
            UpdateMsg(Now & Chr(9) & "New output file created." & vbCrLf, Form_MainNew.RtbColor.Black)
            cnt_newintegrated = cnt_newintegrated + 1
            '  File.Move(Path.GetDirectoryName(Replace(_sFile, "01-Input-B", "03-Backfromtranslation")) & "\" & Path.GetFileNameWithoutExtension(_sFile) & "_" & CurrentLang & ".xliff", Path.GetDirectoryName(Replace(_sFile, "01-Input-B", "03-Backfromtranslation")) & "\(processed)" & Path.GetFileNameWithoutExtension(_sFile) & "_" & CurrentLang & ".xliff")
    ...
    Public Function ExtractXliff() As Boolean Implements iXliff.ExtractXliff
        Try
            Msg = Mod_RMK_xhtml.RMK_to_xliff(_sFile, CloudProjectsettings.Folder_TobeTransalted, CPS.CurrentLang)
            If Msg <> "" Then
    ...
    Public Sub ImportExistingTranslationToDB() Implements iXliff.ImportExistingTranslationToDB
        UpdateMsg(Now & Chr(9) & "DB update not implemented for RMKxhtml" & vbCrLf, Form_MainNew.RtbColor.Red)
    End Sub

    Public Sub ExtractXiffwithExistingTranslation() Implements iXliff.ExtractXiffwithExistingTranslation
        UpdateMsg(Now & Chr(9) & "ExtractXiffwithExistingTranslation not implemented for RMKxhtml" & vbCrLf, Form_MainNew.RtbColor.Red)
    End Sub
End Class
